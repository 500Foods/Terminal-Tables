...
`EXAMPLES.md` and `EXAMPLES_0[1-9].md` embed a coloured SVG screenshot for every scenario
(`images/<label>.svg`, e.g. `images/9-V.svg`). These are **not** hand-drawn — each one is a
literal capture of a real run, generated by piping the *non-`--mono`* output of
`./tables.c/tables` straight into [Oh.sh](https://github.com/500Foods/Oh.sh) (installed as
the `Oh` binary), which converts raw ANSI terminal output into an SVG:

```bash
./tables.c/tables tests/scenarios/suite_09/test_9_V_layout.json \
                   tests/scenarios/suite_09/test_9_V_data.json | Oh -o images/9-V.svg
```

For the one scenario with a `.sh` layout (9-A), render the script's JSON first, then feed
that to `tables`:
...
Command substitution (`$(date)`, etc.) in a static layout's `title`/`footer` strings is
evaluated by the renderer itself at run time (see `evaluate_dynamic_string` in
`tables_render_title.c`/`tables_render_footer.c`, and the `eval echo` calls in
`tables.sh`) — no shell preprocessing of the JSON is needed for those.

**Whenever a rendering bug fix changes a table's actual output** (spacing, box width,
alignment, colors), regenerate every affected image with the command above and rebuild the
C binary first (`make -C tables.c uncompressed`) so the capture reflects the fix — a stale
SVG will keep showing the old, wrong layout even after the code and tests are updated. Also
re-check any prose in the corresponding `EXAMPLES_0X.md` section that describes exact
character counts/widths (e.g. "overhangs by N characters"), since those numbers are
computed from the previous (possibly buggy) output and can go stale silently — they are
