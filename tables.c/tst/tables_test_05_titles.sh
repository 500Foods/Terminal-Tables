...
# Check for --debug and --debug_layout flags
DEBUG_FLAG=""
DEBUG_LAYOUT_FLAG=""
if [[ "$1" == "--debug" ]]; then
    DEBUG_FLAG="--debug"
...
if [[ "$1" == "--debug_layout" ]] || [[ "$2" == "--debug_layout" ]]; then
...
# TestC 5-A: Basic header with title and summaries
cat > "$layout_file" << 'EOF'
{
  "theme": "Red",
...
echo "TestC 5-A: Basic header with title and summaries"
echo "-----------------------------------------------"
"$tables_script" "$layout_file" "$data_file" $DEBUG_FLAG $DEBUG_LAYOUT_FLAG

# TestC 5-B: Header with long title and wrapping description
cat > "$layout_file" << 'EOF'
{
  "theme": "Blue",
...
# TestC 5-C: Header with multiple summary types and centered tags
cat > "$layout_file" << 'EOF'
{
  "theme": "Red",
...
# TestC 5-D: Header with breaking by category and right-aligned title
cat > "$layout_file" << 'EOF'
{
  "theme": "Blue",
...
# TestC 5-E: Elaborate header with full-width title, summaries, and mixed justifications
cat > "$layout_file" << 'EOF'
{
  "theme": "Red",
