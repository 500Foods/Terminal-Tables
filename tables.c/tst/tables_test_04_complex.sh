...
# TestC 4-A: Complex table with summaries, wrapping, and breaking by category
cat > "$layout_file" << 'EOF'
{
  "theme": "Red",
  "columns": [
...
echo "TestC 4-A: Complex table with summaries, wrapping, and breaking by category"
echo "-------------------------------------------------------------------------"
"$tables_script" "$layout_file" "$data_file" $DEBUG_FLAG

# TestC 4-B: Resource-focused table with Kubernetes summaries and delimiter wrapping
cat > "$layout_file" << 'EOF'
{
  "theme": "Blue",
  "columns": [
...
# TestC 4-C: Mixed justification with summaries and centered wrapping
cat > "$layout_file" << 'EOF'
{
  "theme": "Red",
  "columns": [
...
# TestC 4-D: Full-featured table with all summary types and right-aligned tags wrapping
cat > "$layout_file" << 'EOF'
{
  "theme": "Blue",
  "columns": [
...
# TestC 4-E: Elegant table with breaking, wrapping, and minimal summaries
cat > "$layout_file" << 'EOF'
{
  "theme": "Red",
  "columns": [
